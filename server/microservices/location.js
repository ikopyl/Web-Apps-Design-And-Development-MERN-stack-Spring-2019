...
app.get('/location', (req, res) => {
  ...
  const unsupportedParametersMsg =
    'Invalid request: supported query parameters include json, jsonp, xml, csv, and yaml';

  //conditional check to verify that a supported parameter was included and not empty
  if (
    !supportedQueryParameters.includes(req.query.format) &&
    ...
    axios
      ...
      .then((response) => {
        const url =
          ...
          response.data +
          '/' +
          (noQueryParams ? 'json' : req.query.format) +
          '/';
        console.log(url);
